...
# Вывод: меньше всего памяти занимает исходный вариант решения задачи (с использованием переменных). На втором месте
# решение с использованием списка, на треьем с использованием кортежа.
...
for i in range(0, count_num):
    ...
    if check_size(my_num) > sum_memory:
        sum_memory = check_size(my_num)
    while my_num != 0:
...
sum_memory += check_size(find_num) + check_size(sum_result) + check_size(i)
print(f'Размер занятой под переменные памяти равен {sum_memory} байт.')
